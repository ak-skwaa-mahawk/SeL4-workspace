...
/* Statically mapped weight matrices (read-only data segment) */
 ...
 * Input:  int8_t normalized features [-128, 127]
 * Output: classification (0 or 1), confidence Q16.16
 */
int tinyml_infer(const uint8_t input[INPUT_DIM], int32_t *classification, uint32_t *confidence_q16) {
    ...
    if (!input || !classification || !confidence_q16) {
        return -1; // Fault invariant: null capability pointer
    }
