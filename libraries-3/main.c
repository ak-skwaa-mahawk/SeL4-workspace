...
int main(void) {
    ...
    sel4utils_process_config_t client_conf = process_config_default_simple(&simple, CLIENT_IMAGE_NAME, 190);
    ...
    /* Lower root task priority below children so child threads run */
    seL4_TCB_SetPriority(seL4_CapInitThreadTCB, seL4_CapInitThreadTCB, 100);

    /* Suspend root task to yield CPU indefinitely */
